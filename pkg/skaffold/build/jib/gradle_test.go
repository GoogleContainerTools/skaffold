...
func TestBuildJibGradleToDocker(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			artifact:    &latestV1.JibArtifact{Project: "project"},
			commands: testutil.CmdRun(
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			t.Override(&util.DefaultExecCommand, testutil.CmdRunOutErr(
				strings.Join(getCommandGradle(ctx, tmpDir.Root(), &latestV1.JibArtifact{Project: "gradle-test"}).Args, " "),
				test.stdout,
				test.err,
			))
			...
			ws := tmpDir.Root()
			deps, err := getDependenciesGradle(ctx, ws, &latestV1.JibArtifact{Project: "gradle-test"})
			if test.err != nil {
...
func TestGetCommandGradle(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			jibArtifact:      latestV1.JibArtifact{Project: "project"},
			filesInWorkspace: []string{},
			expectedCmd: func(workspace string) exec.Cmd {
...
func TestGenerateGradleBuildArgs(t *testing.T) {
	tests := []struct {
		...
		{description: "multi module", in: latestV1.JibArtifact{Project: "project"}, image: "image", out: []string{"fake-gradleBuildArgs-for-project-for-testTask", "--image=image"}},
		{description: "multi module without tests", in: latestV1.JibArtifact{Project: "project"}, image: "image", skipTests: true, out: []string{"fake-gradleBuildArgs-for-project-for-testTask-skipTests", "--image=image"}},
		...
		},
		{
			...
			in:          latestV1.JibArtifact{Project: "project", BaseImage: "alias"},
			image:       "image",
			deps:        []*latestV1.ArtifactDependency{{ImageName: "img", Alias: "alias"}},
			r:           mockArtifactResolver{m: map[string]string{"img": "img:tag"}},
...
func TestGradleBuildArgs(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			jibArtifact: latestV1.JibArtifact{Flags: []string{"--flag1", "--flag2"}},
			skipTests:   false,
			showColors:  true,
		...
		},
		{
			...
			jibArtifact: latestV1.JibArtifact{Project: "module", Flags: []string{"--flag1", "--flag2"}},
			skipTests:   false,
			showColors:  true,
...
func getGradleArgsFuncFake(t *testutil.T, expectedMinimumVersion string) func(*latestV1.JibArtifact, string, string) []string {
	return func(a *latestV1.JibArtifact, task string, minimumVersion string) []string {
...
func getGradleBuildArgsFuncFake(t *testutil.T, expectedMinimumVersion string) func(string, *latestV1.JibArtifact, bool, bool, string) []string {
	return func(task string, a *latestV1.JibArtifact, skipTests, showColors bool, minimumVersion string) []string {
		t.CheckDeepEqual(expectedMinimumVersion, minimumVersion)
