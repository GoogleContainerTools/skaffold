...
func TestGetDependencies(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			results, err := getDependencies(context.Background(), tmpDir.Root(), exec.Cmd{Args: []string{"ignored"}, Dir: tmpDir.Root()}, &latestV2.JibArtifact{Project: util.RandomID()})
...
func TestGetUpdatedDependencies(t *testing.T) {
	testutil.Run(t, "Both build definitions are created at the same time", func(t *testutil.T) {
		...
		artifact := &latestV2.JibArtifact{Project: util.RandomID()}
...
func TestDeterminePluginType(t *testing.T) {
	tests := []struct {
		...
		{"gradle override", []string{"pom.xml"}, &latestV2.JibArtifact{Type: string(JibGradle)}, false, JibGradle},
		{"maven override", []string{"build.gradle"}, &latestV2.JibArtifact{Type: string(JibMaven)}, false, JibMaven},
	}
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
...
func TestGetProjectKey(t *testing.T) {
	tests := []struct {
		...
		{
			"empty project",
			&latestV2.JibArtifact{},
			"dir",
			projectKey("dir+"),
		},
		{
			"non-empty project",
			&latestV2.JibArtifact{Project: "project"},
			"dir",
