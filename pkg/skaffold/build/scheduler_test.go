...
func TestInOrderForArgs(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			artifactLen: 5,
			expected:    nil,
			err:         fmt.Errorf("couldn't build %q: %v\ncouldn't build %q: %v", "artifact1", fmt.Errorf("failed to build required artifact: %q", "artifact2"), "artifact2", fmt.Errorf("some error occurred while building %q", "artifact2")),
		},
	}
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
