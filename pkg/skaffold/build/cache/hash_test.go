...
type mockDependencyLister struct {
	dependencies []string
}

func (m *mockDependencyLister) DependenciesForArtifact(ctx context.Context, artifact *latest.Artifact) ([]string, error) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			for _, d := range test.dependencies {
				depLister := &mockDependencyLister{dependencies: d}
...
func TestCacheHasher(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			path := originalFile
			depLister := &mockDependencyLister{dependencies: []string{tmpDir.Path(originalFile)}}
			...
			depLister = &mockDependencyLister{dependencies: []string{tmpDir.Path(path)}}
