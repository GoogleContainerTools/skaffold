...
func (c *cache) lookupArtifacts(ctx context.Context, tags tag.ImageTags, artifacts []*latest.Artifact) []chan cacheDetails {
	results := make([]chan cacheDetails, len(artifacts))

	for i := range artifacts {
		results[i] = make(chan cacheDetails, 1)
		i := i
		go func() {
			results[i] <- c.lookup(ctx, artifacts[i], tags[artifacts[i].ImageName])
		}()
	}
