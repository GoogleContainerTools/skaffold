...
func singleArtifactHash(ctx context.Context, depLister DependencyLister, a *latest.Artifact, mode config.RunMode, m platform.Matcher) (string, error) {
	...
	args, err := hashBuildArgs(ctx, a, mode)
	if err != nil {
...
func hashBuildArgs(ctx context.Context, artifact *latest.Artifact, mode config.RunMode) ([]string, error) {
	...
	case artifact.BuildpackArtifact != nil:
		env, err = buildpacks.GetEnv(ctx, artifact, mode)
