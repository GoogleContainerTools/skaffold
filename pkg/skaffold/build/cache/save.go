...
func (c *Cache) RetagLocalImages(ctx context.Context, out io.Writer, artifactsToBuild []*latest.Artifact, buildResults []build.Result) {
...
func (c *Cache) CacheArtifacts(ctx context.Context, artifacts []*latest.Artifact, buildResults []build.Result) error {
	if !c.useCache {
		return nil
	}
	tags := map[string]string{}
