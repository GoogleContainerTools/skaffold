...
func TestLookupLocal(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			details := cache.lookupArtifacts(context.Background(), map[string]string{"artifact": "tag"}, platform.Resolver{}, []*latestV1.Artifact{{
				ImageName: "artifact",
			}})
...
func (m mockHasher) hash(context.Context, *latestV1.Artifact, platform.Resolver) (string, error) {
	return m.val, nil
}
...
func (f failingHasher) hash(context.Context, *latestV1.Artifact, platform.Resolver) (string, error) {
	return "", f.err
}
