...
Each configuration is itself a JSON object of type `util.ContainerDebugConfiguration`, with an
...
type containerTransformer interface {
	...
	Apply(container *v1.Container, config imageConfiguration, portAlloc portAllocator, overrideProtocols []string) (util.ContainerDebugConfiguration, string, error)
}
...
func rewriteProbes(metadata *metav1.ObjectMeta, podSpec *v1.PodSpec) bool {
	...
	if annotation, found := metadata.Annotations[util.DebugProbeTimeoutsAnnotation]; found {
		if annotation == "skip" {
	...
	annotation, found := metadata.Annotations[util.DebugConfigAnnotation]
	if !found {
		logrus.Debugf("skipping probe rewrite on %q: not configured for debugging", metadata.Name)
		return false
	}
	var config map[string]util.ContainerDebugConfiguration
...
func rewriteContainers(metadata *metav1.ObjectMeta, podSpec *v1.PodSpec, retrieveImageConfiguration configurationRetriever, debugHelpersRegistry string) bool {
	...
	if _, found := metadata.Annotations[util.DebugConfigAnnotation]; found {
		return false
	}
	...
	configurations := make(map[string]util.ContainerDebugConfiguration)
	...
	if len(configurations) > 0 {
		...
		metadata.Annotations[util.DebugConfigAnnotation] = encodeConfigurations(configurations)
		return true
	}
	return false
}
...
func transformContainer(container *v1.Container, config imageConfiguration, portAlloc portAllocator) (util.ContainerDebugConfiguration, string, error) {
	...
	next := func(container *v1.Container, config imageConfiguration) (util.ContainerDebugConfiguration, string, error) {
...
func updateForShDashC(container *v1.Container, ic imageConfiguration, transformer func(*v1.Container, imageConfiguration) (util.ContainerDebugConfiguration, string, error)) (util.ContainerDebugConfiguration, string, error) {
...
func performContainerTransform(container *v1.Container, config imageConfiguration, portAlloc portAllocator) (util.ContainerDebugConfiguration, string, error) {
	...
	return util.ContainerDebugConfiguration{}, "", fmt.Errorf("unable to determine runtime for %q", container.Name)
}

func encodeConfigurations(configurations map[string]util.ContainerDebugConfiguration) string {
