...
func (r *SkaffoldRunner) Render(ctx context.Context, out io.Writer, builds []graph.Artifact, offline bool, renderOutputFile string) (manifest.ManifestList, error) {
	...
	renderOut, postRenderFn, err := renderer.WithLogFile(time.Now().Format(deployutil.TimeFormat)+".log", out, r.runCtx.Muted())
	if err != nil {
		return nil, err
	}
	defer postRenderFn()
	ctx, endTrace := instrumentation.StartTrace(ctx, "Render")
	manifestList, errR := r.renderer.Render(ctx, renderOut, builds, offline, renderOutputFile)
	if errR != nil {
