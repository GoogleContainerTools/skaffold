...
func (w withTimings) Build(ctx context.Context, out io.Writer, tagger tag.Tagger, artifacts []*v1alpha2.Artifact) ([]build.Artifact, error) {
	start := time.Now()
	color.Fprintln(out, color.Default, "Starting build...")

	bRes, err := w.Builder.Build(ctx, out, tagger, artifacts)
	if err == nil {
		color.Fprintln(out, color.Default, "Build complete in", time.Since(start))
	}
	return bRes, err
}
...
func (w withTimings) Deploy(ctx context.Context, out io.Writer, builds []build.Artifact) ([]deploy.Artifact, error) {
	start := time.Now()
	color.Fprintln(out, color.Default, "Starting deploy...")

	dRes, err := w.Deployer.Deploy(ctx, out, builds)
	if err == nil {
		color.Fprintln(out, color.Default, "Deploy complete in", time.Since(start))
	}
	return dRes, err
}
...
func (w withTimings) Cleanup(ctx context.Context, out io.Writer) error {
	start := time.Now()
	color.Fprintln(out, color.Default, "Cleaning up...")

	err := w.Deployer.Cleanup(ctx, out)
	if err == nil {
		color.Fprintln(out, color.Default, "Cleanup complete in", time.Since(start))
	}
	return err
}
