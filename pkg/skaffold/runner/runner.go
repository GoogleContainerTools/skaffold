...
func getDeployer(cfg *latest.DeployConfig, kubeContext string, namespace string, defaultRepo string) (deploy.Deployer, error) {
	...
	if cfg.Helm != nil {
		deployers = append(deployers, deploy.NewHelmDeployer(cfg.Helm, kubeContext, namespace, defaultRepo))
	}
	...
	if cfg.Kubectl != nil {
		...
		deployers = append(deployers, deploy.NewKubectlDeployer(cwd, cfg.Kubectl, kubeContext, namespace, defaultRepo))
	}

	if cfg.Kustomize != nil {
		deployers = append(deployers, deploy.NewKustomizeDeployer(cfg.Kustomize, kubeContext, namespace, defaultRepo))
	}
