...
func TestPrintAnalyzeJSON(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			pairs:       []builderImagePair{{jib.Jib{BuilderName: jib.PluginName(latest.JibGradle), Image: "image1", FilePath: "build.gradle", Project: "project"}, "image1"}},
		...
		},
		{
			...
			builders:    []InitBuilder{jib.Jib{BuilderName: jib.PluginName(latest.JibGradle), FilePath: "build.gradle", Project: "project"}, docker.Docker{File: "Dockerfile"}},
...
func fakeValidateJibConfig(path string) []jib.Jib {
	if strings.HasSuffix(path, "build.gradle") {
		return []jib.Jib{{BuilderName: jib.PluginName(latest.JibGradle), FilePath: path}}
	}
	if strings.HasSuffix(path, "pom.xml") {
		return []jib.Jib{{BuilderName: jib.PluginName(latest.JibMaven), FilePath: path}}
	}
	return nil
}
...
func TestResolveBuilderImages(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			buildConfigs:     []InitBuilder{docker.Docker{File: "Dockerfile1"}, jib.Jib{BuilderName: jib.PluginName(latest.JibGradle), FilePath: "build.gradle"}, jib.Jib{BuilderName: jib.PluginName(latest.JibMaven), Project: "project", FilePath: "pom.xml"}},
			images:           []string{"image1", "image2"},
			...
			expectedPairs: []builderImagePair{
				...
				{
					Builder:   jib.Jib{BuilderName: jib.PluginName(latest.JibGradle), FilePath: "build.gradle"},
					ImageName: "image2",
				},
			},
		},
	...
	tests := []struct {
	...
	}{
		{
			...
			builderConfigs: []InitBuilder{
				...
				jib.Jib{BuilderName: jib.PluginName(latest.JibGradle), FilePath: "build.gradle"},
				jib.Jib{BuilderName: jib.PluginName(latest.JibMaven), FilePath: "pom.xml", Image: "not a k8s image"},
			},
			images:        []string{"image1", "image2"},
			expectedPairs: nil,
			expectedBuildersLeft: []InitBuilder{
		...
		},
		{
			...
			builderConfigs: []InitBuilder{
				docker.Docker{File: "Dockerfile"},
				jib.Jib{BuilderName: jib.PluginName(latest.JibGradle), FilePath: "build.gradle", Image: "image1"},
				jib.Jib{BuilderName: jib.PluginName(latest.JibMaven), FilePath: "pom.xml", Image: "image2"},
			},
		...
		},
		{
			...
			builderConfigs: []InitBuilder{
				jib.Jib{BuilderName: jib.PluginName(latest.JibGradle), FilePath: "build.gradle", Image: "image1"},
				jib.Jib{BuilderName: jib.PluginName(latest.JibMaven), FilePath: "pom.xml", Image: "image1"},
			},
			images:        []string{"image1", "image2"},
			expectedPairs: nil,
			expectedBuildersLeft: []InitBuilder{
