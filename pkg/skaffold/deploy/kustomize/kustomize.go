...
func (k *Deployer) Deploy(ctx context.Context, out io.Writer, log *logrus.Logger, builds []graph.Artifact) error {
	...
	manifests, err := k.renderManifests(childCtx, out, log, builds)
	if err != nil {
...
func (k *Deployer) renderManifests(ctx context.Context, out io.Writer, log *logrus.Logger, builds []graph.Artifact) (manifest.ManifestList, error) {
