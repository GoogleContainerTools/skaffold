...
func (h *Deployer) Render(ctx context.Context, out io.Writer, builds []graph.Artifact, offline bool, filepath string) error {
	...
	for _, r := range h.Releases {
		...
		args := []string{"template", releaseName, helm.ChartSource(r)}
		...
		if err != nil {
			return helm.UserErr("construct override args", err)
		}
		...
		if err := helm.Exec(ctx, h, outBuffer, false, helmEnv, args...); err != nil {
			return helm.UserErr("std out err", fmt.Errorf(outBuffer.String()))
		}
