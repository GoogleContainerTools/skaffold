...
func NewDeployer(ctx context.Context, cfg Config, labeller *label.DefaultLabeller, h *latestV1.HelmDeploy) (*Deployer3, error) {
	hv, err := binVer(ctx)
	if err != nil {
...
func (h *Deployer3) deployRelease(ctx context.Context, out io.Writer, releaseName string, r latestV1.HelmRelease, builds []graph.Artifact, valuesSet map[string]bool, helmVersion semver.Version, chartVersion string) ([]types.Artifact, error) {
	var err error
	opts := installOpts{
...
func (h *Deployer3) packageChart(ctx context.Context, r latestV1.HelmRelease) (string, error) {
