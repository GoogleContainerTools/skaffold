...
func NewDeployer(cfg Config, step latest.DeployType, labels map[string]string) (*Deployer, error) {
	defaultNamespace := ""
	if step.KubectlDeploy.DefaultNamespace != nil {
		var err error
		defaultNamespace, err = util.ExpandEnvTemplate(*step.KubectlDeploy.DefaultNamespace, nil)
		if err != nil {
			return nil, err
		}
	}
	...
	return &Deployer{
		KubectlDeploy:      step.KubectlDeploy,
		workingDir:         cfg.GetWorkingDir(),
		globalConfig:       cfg.GlobalConfig(),
		defaultRepo:        cfg.DefaultRepo(),
		kubectl:            NewCLI(cfg, step.KubectlDeploy.Flags, defaultNamespace),
