...
func TestNewSyncItem(t *testing.T) {
	...
	tests := []struct {
		...
		// manual sync cases
		{
			...
			artifact: &latestV2.Artifact{
				ImageName: "test",
				Sync: &latestV2.Sync{
					Manual: []*latestV2.SyncRule{{Src: "*.html", Dest: "."}},
				},
				Workspace: ".",
			},
			builds: []graph.Artifact{
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			t.Override(&SyncMap, func(context.Context, *latestV2.Artifact, docker.Config) (map[string][]string, error) {
			...
			t.Override(&jib.GetSyncDiff, func(context.Context, string, *latestV2.JibArtifact, filemon.Events) (map[string][]string, map[string][]string, error) {
...
func TestSyncMap(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			syncMap, err := SyncMap(context.Background(), &latestV2.Artifact{ArtifactType: test.artifactType}, nil)
...
func TestInit(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			artifact:    &latestV2.Artifact{Sync: &latestV2.Sync{}},
			shouldInit:  false,
		},
		{
			description: "sync on, auto on, non-jib",
			artifact:    &latestV2.Artifact{Sync: &latestV2.Sync{Auto: util.BoolPtr(true)}},
			shouldInit:  false,
		},
		{
			description: "sync on, auto on, jib",
			artifact:    &latestV2.Artifact{ArtifactType: latestV2.ArtifactType{JibArtifact: &latestV2.JibArtifact{}}, Sync: &latestV2.Sync{Auto: util.BoolPtr(true)}},
			shouldInit:  true,
			initErrors:  false,
		},
		{
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			isCalled := false
			t.Override(&jib.InitSync, func(ctx context.Context, workspace string, a *latestV2.JibArtifact) error {
				isCalled = true
				if test.initErrors {
			...
			artifacts := []*latestV2.Artifact{test.artifact}
