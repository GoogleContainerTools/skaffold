...
type LocalDaemon interface {
	...
	ContainerInspect(ctx context.Context, id string) (client.ContainerInspectResult, error)
	...
	ImageInspectWithRaw(ctx context.Context, image string) (client.ImageInspectResult, []byte, error)
...
func (l *localDaemon) VolumeCreate(ctx context.Context, opts client.VolumeCreateOptions) (client.VolumeCreateResult, error) {
	return l.apiClient.VolumeCreate(ctx, opts)
}

func (l *localDaemon) VolumeRemove(ctx context.Context, id string) error {
	_, err := l.apiClient.VolumeRemove(ctx, id, client.VolumeRemoveOptions{Force: false})
	return err
}

func (l *localDaemon) ContainerInspect(ctx context.Context, id string) (client.ContainerInspectResult, error) {
	return l.apiClient.ContainerInspect(ctx, id, client.ContainerInspectOptions{})
}

func (l *localDaemon) ContainerList(ctx context.Context, options client.ContainerListOptions) (client.ContainerListResult, error) {
...
func (l *localDaemon) Run(ctx context.Context, out io.Writer, opts ContainerCreateOpts) (<-chan container.WaitResponse, <-chan error, string, error) {
	...
	if opts.Wait {
		res := l.apiClient.ContainerWait(ctx, c.ID, client.ContainerWaitOptions{Condition: container.WaitConditionNotRunning})
		return res.Result, res.Error, c.ID, nil
	}
...
func (l *localDaemon) NetworkList(ctx context.Context, opts client.NetworkListOptions) (client.NetworkListResult, error) {
...
func (l *localDaemon) isAlreadyPushed(ctx context.Context, ref, registryAuth string) (bool, string, error) {
	localImage, err := l.apiClient.ImageInspect(ctx, ref)
	if err != nil {
		return false, "", err
	}
	...
	remoteImage, err := l.apiClient.DistributionInspect(ctx, ref, client.DistributionInspectOptions{EncodedRegistryAuth: registryAuth})
	if err != nil {
		return false, "", err
	}
...
func (l *localDaemon) Pull(ctx context.Context, out io.Writer, ref string, platform v1.Platform) error {
	...
	rc, err := l.apiClient.ImagePull(ctx, ref, client.ImagePullOptions{
		...
		Platforms: []ocispec.Platform{ocispec.Platform{
...
func (l *localDaemon) ImageID(ctx context.Context, ref string) (string, error) {
	image, err := l.apiClient.ImageInspect(ctx, ref)
	if err != nil {
...
func (l *localDaemon) ImageExists(ctx context.Context, ref string) bool {
	_, err := l.apiClient.ImageInspect(ctx, ref)
	return err == nil
}

func (l *localDaemon) ImageInspectWithRaw(ctx context.Context, image string) (client.ImageInspectResult, []byte, error) {
	raw := &bytes.Buffer{}
	res, err := l.apiClient.ImageInspect(ctx, image, client.ImageInspectWithRawResponse(raw))
	return res, raw.Bytes(), err
}
...
func (l *localDaemon) ImageList(ctx context.Context, ref string) ([]image.Summary, error) {
	f := client.Filters{}
	f.Add("reference", ref)
	resp, err := l.apiClient.ImageList(ctx, client.ImageListOptions{
		Filters: f,
	})
	if err != nil {
		return nil, err
...
func (l *localDaemon) DiskUsage(ctx context.Context) (uint64, error) {
	res, err := l.apiClient.DiskUsage(ctx, client.DiskUsageOptions{
		Containers: true,
		Images:     true,
		BuildCache: true,
		Volumes:    true,
	...
	usage := res.Containers.TotalSize + res.Images.TotalSize + res.BuildCache.TotalSize + res.Volumes.TotalSize
