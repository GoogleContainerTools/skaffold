...
func TestParseJson(t *testing.T) {
	tests := []struct {
		name     string
		config   latestV1.JSONParseConfig
		line     string
		expected string
	}{
		{
			name:     "standard json parse",
			config:   latestV1.JSONParseConfig{Fields: []string{"message", "severity", "timestampSeconds"}},
		...
		},
		{
			...
			config:   latestV1.JSONParseConfig{Fields: []string{"message", "severity", "invalid"}},
			line:     "{\"timestampSeconds\":1643740871,\"timestampNanos\":446000000,\"severity\":\"INFO\",\"message\":\"Hello World\"}\n",
			expected: "message: Hello World, severity: INFO\n",
		},
		{
			name:     "all specified fields not found in json object",
			config:   latestV1.JSONParseConfig{Fields: []string{"invalid"}},
			line:     "{\"valid\":\"Hello World\"}\n",
			expected: "{\"valid\":\"Hello World\"}\n",
		},
		{
			name:     "non json line input",
			config:   latestV1.JSONParseConfig{Fields: []string{"message", "severity"}},
			line:     "Hello World!\n",
