...
func (t *envTemplateTagger) GenerateTag(ctx context.Context, image latest.Artifact) (string, error) {
	...
	// It will be replaced by <no value> if no other Go template method handles the missing key. Tagging will fail at a
	// later stage. This gives us an opportunity to handle missing keys with other Go template methods. For
	// example, the default method can be used to set a default value for a missing key. The evaluation of
	// {{default "bar" .FOO}} should be "bar" and tagging should succeed if the environment variable .FOO does not exist.
