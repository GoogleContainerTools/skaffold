...
func TestIgnoreCancelled(t *testing.T) {
	tcs := []struct {
		description string
		prev        []proto.StatusCode
		new         []proto.StatusCode
		expected    [][]proto.StatusCode
	}{
		{
			description: "status cancelled set when no prev state set",
			prev:        []proto.StatusCode{0},
			new:         []proto.StatusCode{801, 850, 845},
			expected:    [][]proto.StatusCode{{801, 850, 845}},
		},
		{
			description: "status cancelled set when prev state is successful",
			prev:        []proto.StatusCode{200, 210, 250},
			new:         []proto.StatusCode{800, 810, 850},
			// new status code is set
			expected: [][]proto.StatusCode{{800, 810, 850}, {800, 810, 850}, {800, 810, 850}},
		},
		{
			description: "status cancelled ignored when prev state is neither success nor set",
			prev:        []proto.StatusCode{910, 3, 500},
			new:         []proto.StatusCode{800, 810, 850},
			// prev status code is retained when new is range of 800 to 850
			expected: [][]proto.StatusCode{{910, 910, 910}, {3, 3, 3}, {500, 500, 500}},
		},
	}
	for _, tc := range tcs {
		t.Run(tc.description, func(t *testing.T) {
			for i := range tc.prev {
				for j := range tc.new {
					testutil.CheckDeepEqual(t, tc.expected[i][j], ignoreCancelled(tc.prev[i], tc.new[j]))
				}
			}
		})
	}
