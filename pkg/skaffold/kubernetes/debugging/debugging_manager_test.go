...
func TestDebuggableContainerManager(t *testing.T) {
	testutil.Run(t, "simulation", func(t *testutil.T) {
		startCount := 0
		terminatedCount := 0
		t.Override(&notifyDebugContainerStarted, func(podName string, containerName string, namespace string, artifactImage string, runtime string, workingDir string, ports map[string]uint32) {
			startCount++
		})
		t.Override(&notifyDebugContainerTerminated, func(podName string, containerName string, namespace string, artifactImage string, runtime string, workingDir string, ports map[string]uint32) {
			terminatedCount++
		})
		pod := v1.Pod{
		...
		m := &DebuggableContainerManager{output: &bytes.Buffer{}, active: make(map[string]string)}
