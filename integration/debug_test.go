...
func TestDebugEventsRPC_StatusCheck(t *testing.T) {
	...
	rpcAddr := randomPort()
	skaffold.Debug("-vdebug", "--enable-rpc", "--rpc-port", rpcAddr).InDir("testdata/jib").InNs(ns.Name).RunBackground(t)
...
func TestDebugEventsRPC_NoStatusCheck(t *testing.T) {
	...
	rpcAddr := randomPort()
	skaffold.Debug("-vdebug", "--enable-rpc", "--rpc-port", rpcAddr, "--status-check=false").InDir("testdata/jib").InNs(ns.Name).RunBackground(t)
