...
func NewCmdDev(out io.Writer) *cobra.Command {
	...
	return NewCmd(out, cmdUse).
		...
		WithFlags(func(f *pflag.FlagSet) {
			f.StringVar(&opts.BuildTrigger, "build-trigger", "polling", "How are changes detected? (polling, manual or notify)")
			f.BoolVar(&opts.ManualDeploy, "manual-deploy", false, "Whether deploys are done automatically or manually (through the API)")
